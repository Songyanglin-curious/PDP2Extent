...
function loadContextMenu(xmlpath,xslpath) {
  ...
  if(xmlpath != "") {
    ...
    contextMenu.style.left = window.event.x + document.body.scrollLeft -3
    
		if(ValignD > MenuHeight){
			contextMenu.style.top = window.event.y + document.body.scrollTop -5
		}	else{
			contextMenu.style.top = window.event.y < MenuHeight? document.body.scrollTop + 5:document.body.scrollTop + window.event.y-MenuHeight+10
		}    		
