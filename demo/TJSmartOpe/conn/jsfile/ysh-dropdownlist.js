...
function DropDownList($, textControl, valueControl, labelControl) {
    ...
    this.iframeId = textControl.split("_txtText")[0] + "_iframe";
...
//        $(document.body).append(selectList);
        ...
        //        this.bindEvent(this);
        $(function () { $(document.body).append(selectList); obj.bindEvent(obj); }); //等body装载完再执行，不然body.appendChild会有问题        
    ...
    this.refresh = function (d, s, it, f) {
        ...
        var textField = obj.property.textField.toLowerCase();
        var valueField = obj.property.valueField.toLowerCase();
        ...
        if (obj.data == null) {//update by subo 2013-01-24 由于上一行的操作，导致这一行的判断永远为true，每次都会执行降低效率，所以注释掉
            $.executeAjax(obj.url, d, function (result) {
                ...
                obj.cols = result.col;
                $('#' + obj.selectListID).parent().parent().data("ddlData", obj.data);
            });
        }
        if (obj.data.length == 0) {
            var o = new Array();
            o[obj.property.textField.toLowerCase()] = "";
            o[obj.property.valueField.toLowerCase()] = "";
            obj.data[0] = o;
            //obj.vControl.val('');
            //selListContent.hide();
        ...
        var selectList = $('#' + obj.selectListID);
        //selectList.hide();
        //add by gud 20130106，添加的代码请不要加在这段之后，要加也加在前边
        if (Ysh.isIE) {
            var frmFilter = document.createElement("<iframe id='" + this.iframeId + "' scrolling='no' frameborder='0' src='' style=\"position:absolute;visibility:inherit;top:0;left:0; z-index: -1;'filter='progid:dximagetransform.microsoft.alpha(style=0,opacity=0)';\"></iframe>");
    ...
    this.setSelected = function (rIndex) {
        ...
        var tableRows = $("#" + obj.selectListID).find('table').find('tr');
        obj.tControl.val(tableRows.eq(rIndex).children("td").eq(0).text());
        obj.vControl.val(tableRows.eq(rIndex).children("td").eq(0).attr('v'));
    };
    this.setSelectedValue = function (v) {
        ...
        if (obj.property.getValueType.toLocaleLowerCase() == 'id') {
            var bExists = false;
            if ((null != obj.data) && (obj.property.multiSelect != 1)) {
                for (var i = 0; i < obj.data.length; i++) {
                    if (obj.data[i][obj.property.valueField.toLowerCase()] == v) {
                        bExists = true;
                        obj.tControl.val(obj.data[i][obj.property.textField.toLowerCase()]);
                        obj.lControl.text(obj.data[i][obj.property.textField.toLowerCase()]);
                        break;
                    }
                }
            }
    ...
    this.SetIframeHeight = function () {
        ...
        var belowHeight = $("#" + this.selectListID).find("#next").length == 1 ? 22 : 0;
        var height = $("#" + this.selectListID).find("tr").length * 18 + belowHeight;
    ...
    $.extend({
        page: function (divId, pagesize, pageindex, data, textField, valueField, obj) {
            ...
            function bar() {
                ...
                $("#fir").bind("click", {}, function () { event.cancelBubble = true; showFir(); });
                $("#pre").bind("click", {}, function () { event.cancelBubble = true; showPre(); });
                $("#next").bind("click", {}, function () { event.cancelBubble = true; showNext(); });
                $("#lat").bind("click", {}, function () { event.cancelBubble = true; showLast(); });
                $("#go").bind("click", {}, function () { event.cancelBubble = true; showGo(); });
            }
...
//                    var templist = [];
//                    var p = selListContent.parent();
//                    while (p[0].style) {
//                        templist.push(p[0].style.position);
//                        p = p.parent();
//                    }
//                    alert(templist);
                    var A_top = 0, A_left = 0;
                    //if ($("#dvDropDownList").parent().css("position") != "absolute") {
                    if (true) {//下拉框移到外边，已经不受父窗口的影响
