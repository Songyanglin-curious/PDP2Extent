    ...
    methods: {
        ...
        clickDisplayMenu: function (item, args) {
            ...
            if (item.id == "border_3d") {
                if (item.on == "1")
                    MapOpeInst.postMessage({ eventType: 'menuope', menuname: 'showDistric3DShadow', selstate: (item.on == "1"), data: { notNeedRota: true } });
                else
                    MapOpeInst.postMessage({ eventType: 'menuope', menuname: 'showDistric3DShadow', selstate: (item.on == "1"), data: { noFlyBack: true } });
                return;
            }
...
floatDataInst.showData = function (type, data, left, top, wnd) {
    ...
    var exes = [];
    if (time >= new Date()) { //预测
        exes.push({ type: 'read', db: 'SGC', sql: "sgc/tq:GetNearestForePoint", args: [data.longitude, data.latitude] });
        exes.push({ type: 'read', db: 'SGC', sql: "sgc/tq:GetForeDataByPoint", args: [{ ref: 0, value: 0 }, time, time.getFullYear()] });
    } else {
        //exes.push({ type: 'read', db: 'SGC', sql: "sgc/tq:GetNearestHisPoint", args: [data.longitude, data.latitude] });
        //exes.push({ type: 'read', db: 'SGC', sql: "sgc/tq:GetHisDataByPoint", args: [{ ref: 0, value: 0 }, time] });
        exes.push({ type: 'read', db: 'SGC', sql: "sgc/tq:GetDataByPos", args: [data.longitude, data.latitude, time, time.getFullYear()] });
    }
    $.ajax({
            ...
            if (this.state != 0) {
                ...
                */
                ...
                AppHelper.readValidTimes(options, now, start, end, function (times) {
                    ...
                    vMain.playerSpeed = options.speed || 1;//2;
...
var vMain = new Vue({
    ...
    data: {
        ...
        buttons: [//type:0,1 - 图,2 - 页,3 - 其他
            ...
            },//jxp
            {
                id: "icing", state: 0, type: 2, needClear: true, canPlay: true, speed: 5, name: "覆冰", click: function () {
                    vMain.changePage("icing", "覆冰", "sgc_tq_icing_list");
                }, getImage: function () { return this.state != 0 ? "/i/sgc/weather.png" : "/i/sgc/weather_dis.png"; }
            },//icing
            {
                id: "fire", state: 0, type: 3, needClear: true, name: "山火", click: function () {
                    vMain.changePage("fire", "山火", "sgc_tq_fire_list");
                    return;
                    var legend = "none";
                    if (this.state) {
                        var ver = "1.20.1026.1";
        ...
        realbuttons_all: [
            ...
            {
                isHide: true, id: "realvol", state: 0, name: "母线电压", img: "/i/sgc/module/realvol.png",
                /*click: function () {
                    alert("正在优化调试中，敬请期待");
                },*/
                hide: function () {
            ...
            {
                isHide: true, id: "realsection", state: 0, name: "断面路况", page: "sgc_section_real", img: "/i/sgc/module/section.png", isShowing: false, isRefreshing: false, legend: "/i/sgc/section/legend.png", showType: 1,
                hide: function () {
                    if (this.timerId) {
                        window.clearTimeout(this.timerId);
                        this.timerId = null;
            ...
            {
                isHide: true, id: "plantpower", state: 0, name: "电厂出力", img: "/i/sgc/module/realpower.png", legend: "/i/sgc/legend/dianchangchuli.png",
                hide: function () {
    ...
    computed: {
        ...
        legendImageList: function () {
            var arr = ['/i/sgc/legend/main.png?v=1.21.122.1'];
        ...
        showLegend: function () {
            return this.legendImg.indexOf("none") < 0;
        },
    ...
    methods: {
        ...
        init: function () {
            ...
            if (login.mode == "bigscreen") {
                this.getApp("uhvdcmap").state = 1;
                this.getApp("uhvdcmap").click();
                this.getApp("uhvacmap").state = 1;
                this.getApp("uhvacmap").click();
            }
            this.inited = true;
            this.$refs.psys.changeSectionTime = function () {
                vMain.changeSectionTime(vMain.sectiontime);
            }
            this.$refs.ssys.changeSectionTime = function () {
                vMain.changeSectionTime(vMain.sectiontime);
            }
            var exes = [];

            exes.push({ type: "dll", dll: "SGCLib:SGCLib.Settings.GetSettings", args: [login.userid] });
            if (login.mode != "bigscreen") {
                exes.push({ type: "read", db: "SGC", sql: "sgc/sgc:IsAdmin", args: [login.userid] });
                exes.push({ type: "dll", dll: "PDP2.0:PDP2.ModulePriv.GetModuleList", args: [login.usernum] });
                exes.push({ type: "read", db: "ConnMain", sql: "SearchCommonSql:GetSceneList", args: [login.userid] });
                exes.push({ type: "read", db: "ConnMain", sql: "SearchCommonSql:GetSysSceneList", args: [] });
            }

            var vm = this;
            PDP.exec(exes, function (data) {
                if (!data.check("系统初始化", true))
                    return;
                data = data.value;
                userSettings.init(data[0]);
                if (login.mode != "bigscreen") {
                    login.isAdmin = ((data[1].length > 0) && (data[1][0][1]));
                    var modules = vm.$refs.modules;
                    modules.init(data[2][0], data[2][1], login.isAdmin);
                    /*
                    var testModule =
                        {
                            img: "/i/sgc/module/project.png", text: "重点工程", url: "app:project"
                        }
                    modules.fullModule(testModule);

                    //Ysh.Array.getLast(vm.$refs.modules.modules).subs.push(testModule);
                    modules.modules.push(testModule);
                    */
                    vMain.addModule(modules, "电网信息", "三跨", "3k", "/i/sgc/module/sankua.png");
                    showingMenu.setScenes(data[3], data[4]);
                    MapOpeInst.postMessage({ eventType: "menuope", menuname: "getAdminInfo", selstate: login.isAdmin });
                }
                //SGCDB.init();
                vMain.loading = false;
                vMain.legendShowState = false;
                stationFilter.clickDisplayMenu(stationFilter.getDisplayMenu("isShowByRegion"))
                //if (SGCChaoliu)
                //    SGCChaoliu.start();
                var app = Ysh.Request.get("app");
                if (app) {
                    var appid = "", args = "";
                    var idx = app.indexOf(",");
                    if (idx > 0) {
                        appid = app.substr(0, idx);
                        args = app.substr(idx + 1, app.length - idx - 1);
                    } else {
                        appid = app;
                    }
                    vMain.gotoApp(appid, args);
                }
                cityData.onGridChanged = function (force) {
                    return;
                    var showZhidiao = (this.selected.indexOf("0101990101") >= 0);
                    if (!force) {
                        if (ProjectSGC.Map.isRunOtherModeApp())
                            return;
                        if (showZhidiao == this.zhidiao)
                            return;
                    }
                    this.zhidiao = showZhidiao;
                    if (!showZhidiao) {
                        ProjectSGC.Map.setMode("zhidiaomain", "");
                        ProjectSGC.Map.setMode("zhidiao", "");
                        return;
                    }
                    var dlls = [{ type: "read", db: "SGC", sql: "sgc/huabei:GetPlantByDcc", args: ["0021990101"] }];
                    PDP.exec(dlls, function (data) {
                        if (!data.check("获取直调信息", true)) return;
                        var lineids = Ysh.Array.col(data.value[0], 0);
                        var stationids = [];
                        ProjectSGC.Map.setMode("zhidiaomain", "main");
                        console.log("设置直调other模式");
                        ProjectSGC.Map.setMode("zhidiao", "other");
                        ModelList.require(["line"], function () {
                            console.log("显示直调厂站线");
                            ProjectSGC.Map.showStationLines(lineids, stationids, "zhidiao");
                        });
                    });
                }
                ProjectSGC.Map.defaultModeApps = ["zhidiao", "zhidiaomain"];
                ProjectSGC.Map.returnDefaultMode = function () {
                    cityData.onGridChanged(true);
                }
                stationFilter.clickDisplayMenu(stationFilter.getDisplayMenu("border_3d"));
            });
            searchlistOnLoad();
            SearchCommonOnLoad();
            //绑定弹窗事件
            smallWindow.Init();
            setTimeout(function () {
                $.ajax({ url: "/Common/InitSearch", type: "post", dataType: "json", async: true, error: function (data, status, err) { }, success: function (data) { console.log("initsearch ok!") } });
            }, 5000);
        },
