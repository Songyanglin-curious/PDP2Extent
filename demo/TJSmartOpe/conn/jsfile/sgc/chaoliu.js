...
var SGCChaoliu = {
    ...
    init: function () {
        /*this.worker = new Worker("/conn/jsfile/sgc/datawork.js");
        ...
        ModelList.require("station", function () {
            $.ajax({
                url: "/conn/ashx/AskHandler.ashx?m=Read2&conn=SGC&xml=sgc/chaoliu:GetAllEnds",
                dataType: "json",
                type: "post",
                data: {},
                async: true,
                success: function (data) {
                    if ((data.length != 2) || (!data[0])) {
                        window.setTimeout(function () { SGCChaoliu.init(); }, 1000);
                        return;
                    }
                    SGCChaoliu._inited = true;
                    var lst = data[1];
            ...
            PDP.exec(exes, function (ret) {
                ...
                SGCChaoliu._inited = true;
                SGCChaoliu.readEnds(ret.value[0], SGCChaoliu._endsAc);
                SGCChaoliu.readEnds(ret.value[1], SGCChaoliu._endsDc);
            });
        });
    },
    getLineMeasTailItem: function (combines, id, value, lst, lstTail, ends, bCombine, skipByVol) {
        ...
        var deletechaoliu = ["120699010100000009", "120699010100000010", "120699010100000011", "120699010100000012"];
        if ((!b) && (deletechaoliu.indexOf(lineid) < 0)) {
            f(lst, o);
        }
    ...
    readData: function (ret) {
        ...
        for (var i = 0; i < ret.length; i++) {
            if (this.showDcEx) {
                if (i == ret.length - 1) {
                    var arrDcEx = this.getRecentMeas(ret[i]);
                    this.getLineMeasTail(lst, lstTail, arrDcEx, this._endsDc, false);
                    continue;
                }
            }
            if (i == nDc) {
