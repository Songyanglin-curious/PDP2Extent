...
function redrawVML() {
    ...
    for (var i = 0; i < g_Flow.getStepCount(); i++) {
        var Step = g_Flow.step(i);
        vml += Step.getVML(getStepEvents(Step), g_Flow.cfg);
    }
    for (i = 0; i < g_Flow.getActionCount(); i++) {
        var Action = g_Flow.action(i);
        vml += Action.getVML(getActionEvents(Action), g_Flow.cfg);
    }
    vml += '<v:line id="theHidden" style="z-index:50;position_dump:absolute;" from="0,0" to="0,0" strokecolor="blue" strokeweight="2"><v:stroke EndArrow="Classic"/></v:line><v:line id="theLinkLine" style="z-index:50;position_dump:absolute;" from="0,0" to="0,0" strokecolor="blue" strokeweight="2"><v:stroke EndArrow="Classic"/></v:line>';
    VML_FLOWOBJ = document.all.FlowVML;
    VML_FLOWOBJ.innerHTML = vml;
    VML_LINKLINE = document.all.theLinkLine;
    drawTreeView();
    FOCUSEDOBJID = "";
    FOCUSEDOBJTYPE = "";
}
...
function setFlowInfo(info) {
    switch (info) {
        case 3:
            window.showModalDialog("_setFlowYaodian.aspx?flowid=" + g_Flow.id + "&a=" + new Date(), window, "dialogWidth:373px; dialogHeight:460px; center:yes; help:no; resizable:no; status:no");
break;
        case 4:
            window.showModalDialog("_setFlowBackType.aspx?flowid=" + g_Flow.id + "&a=" + new Date(), window, "dialogWidth:373px; dialogHeight:460px; center:yes; help:no; resizable:no; status:no");
            break;
        default:
            break;
    }
...
function setStepInfo(info, stepId) {
    switch (info) {
        case 3:
            window.showModalDialog("_setStepYaodian.aspx?flowid=" + g_Flow.id + "&nodeid=" + stepId + "&a=" + new Date(), window, "dialogWidth:373px; dialogHeight:460px; center:yes; help:no; resizable:no; status:no");
            break;
        case 4:
            window.showModalDialog("_setStepBackType.aspx?flowid=" + g_Flow.id + "&nodeid=" + stepId + "&a=" + new Date(), window, "dialogWidth:373px; dialogHeight:460px; center:yes; help:no; resizable:no; status:no");
            break;
        default:
            break;
    }
...
function editFlowDetail(flowId) {
    window.showModalDialog(location.pathname + "?s=1&flowid="+flowId+"&a"+new Date(), "", "dialogWidth:900px; dialogHeight:700px; center:yes; help:no; resizable:no; status:no");
}
