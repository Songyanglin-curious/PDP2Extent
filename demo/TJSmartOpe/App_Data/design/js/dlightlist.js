...
ProjectSGC.Global.getMainObject("vMain").setLegend("/i/sgc/legend/light.png", true);
this.closePage = function () {
    ProjectSGC.Map.highLight([], []);
    this.closeLine();
    var main = ProjectSGC.Global.getMainObject("vMain");
    if (!main) return;
    main.setLegend("/i/sgc/legend/light.png", false);
...
this.closeLine = function () {
    // this.bar.hide = false;
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showStationLineByOpacity", selstate: false, data: { lineIDs: [], stationIDs: [] } }, "light");
    this.showBar();
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showImageIcon", selstate: false, data: { type: "lighttower" } });
    var main = ProjectSGC.Global.getMainObject("vMain");
    if (!main) return;
...
this.showLineLight = function (id, st, et) {
    ...
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showStationLineByOpacity", selstate: true, data: { lineIDs: [id], stationIDs: ['1'] } }, "light");
    (ProjectSGC.Global.getMainObject("bottomMenuInst") || {}).noMenu = id;
    ProjectSGC.Map.locate("", id);
    ProjectSGC.Map.hideIcon("light");
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showImageIcon", selstate: false, data: { type: "lighttower" } });
    var args = {
    ...
    return;
    PDP.read("SGAPP", "sgc/light:GetLineTower", [id, st, et], function (data) {
        if (!data.isOK) return;
        ...
        ProjectSGC.Map.postMessage({
            eventType: "menuope", menuname: "showImageIcon", selstate: true, data: { type: "lighttower", paddingtop: -18, images: { width: 30, height: 37.5, imgUrl: "/textures/coin/Pin.png" }, locateData: msg }
        });
...
this.refreshTimeRange = function () {
    switch (parseInt(this.timeType)) {
        case -1:
            this.st = new Date();
            this.et = Ysh.Time.add('mi', 30, this.st);
            break;
        case 0:
...
this.initRequest1 = function (m) {
    var bTimeInit = false;
    if (this.inittime) {
        var _this = this;
        PDP.read("SGAPP", "sgc/light:GetTimesByLineTime", [this.initline, this.inittime], function (data) {
            if (data.check("获取线路雷电时间", true)) {
                data = data.value;
                if (data.length == 0) {
                    alert("未获取到线路雷电时间！");
                } else {
                    bTimeInit = true;
                    _this.moreTime = Ysh.Time.getTimeStart(new Date(Ysh.Time.parseDate(data[0][0])), "mi");
                    _this.moreTime1 = Ysh.Time.getTimeStart(Ysh.Time.add('ss', 59, new Date(Ysh.Time.parseDate(data[0][1]))), "mi");
                    _this.timeType = 3;
                    _this.curtype = 2;
                    _this.statType = 2;
                    _this.searchMoreTime(true);
...
this.help = function (v) {
    ...
    if (v == "1") {
        arr.push("告警：");
        arr.push("二级告警：三分钟之内在线路1公里范围内落雷次数大于2次且小于等于5次");
        arr.push("一级告警：三分钟之内在线路1公里范围内落雷次数大于5次")
    } else if (v == "2") {
        //arr.push("关联故障：告警第一次落雷和最后一次落雷时间范围内记录的故障原因分类为自然灾害-雷电的故障");
        arr.push("关联故障：");
        arr.push("选定时间范围内且在落雷时间前后三分钟范围内记录的故障原因分类为自然灾害-雷电的故障");
    } else {
        arr.push("概况：");
        //arr.push("雷电个数：时间范围内全国落雷次数");
        arr.push("影响厂站：厂站1公里范围内有落雷");
        arr.push("影响线路：线路1公里范围内有落雷");
        arr.push("雷电排行：不包含云南、广西、广东、贵州、海南、台湾、香港、澳门的落雷");
    }
...
this.refreshLine = function (m) {
    var _this = this;
    PDP.read("SGAPP", "sgc/light:GetLineStat",[this.st,this.et], function (ret) {
        if (!ret.check("获取雷电数据", true)) return;
        _this.ds1 = ret.value;
...
this.createBar = function () {
    ...
    var vMain = ProjectSGC.Global.getMainObject("vMain");
    if (!vMain) return;
    this.bar = vMain.floatPage("sgc_tq_light_region", { defaultlevel: this.defaultlevel, defaultcity: this.defaultcity, defaultname: this.defaultname, needrefresh: !(this.initline && this.inittime) ,main:this }, (new Date()).getTime());
}
