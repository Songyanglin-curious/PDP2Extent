...
this.changeTimeButtons = function () {
    if (this.showTime) {
        this.timebuttons = [["today", "今日"], ["thisweek", "本周"], [":,d,1", "未来第一天"], [",d,1:,d,2", "未来第二天"], [",d,2:,d,3", "未来第三天"]];        
    } else
        this.timebuttons = [["today", "今日"], [":,d,1", "未来第一天"], [",d,1:,d,2", "未来第二天"], [",d,2:,d,3", "未来第三天"]];

    this.timebuttons = [["today", "今日"], [":,d,1", "未来第一天"], [",d,1:,d,2", "未来第二天"], [",d,2:,d,3", "未来第三天"],[",d,3:,d,4", "未来第四天"],[",d,4:,d,5", "未来第五天"],[",d,5:,d,6", "未来第六天"],[",d,6:,d,7", "未来第七天"]];
}
this.changeShowHistory = function () {
    this.showTime = !this.showTime;
    this.changeTimeButtons();
}
this.changeTimeButtons();
this.closePage = function () {
    ProjectSGC.Map.highLight([], [], "");
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showTowerLine", data: { clear: true } });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "ShowGlowForIceLine", selstate: false });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showPartialTowerLine", selstate: false });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showImageIcon", selstate: false, data: { type: "icingtower" } });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showStationLineByOpacity", selstate: false, data: { lineIDs: [], stationIDs: [] } }, "icing");
...
this.locateRow = function (row) {
    ...
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "ShowGlowForIceLine", selstate: false });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showPartialTowerLine", selstate: false });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showStationLineByOpacity", selstate: true, data: { lineIDs: [lineid], stationIDs: ["1"] } }, "icing");
    //ProjectSGC.Map.locate("LINE", lineid);
    ProjectSGC.Map.locate("LINE", lineid, null, true, function (msg) { msg.locateType = 14; msg.padding = [10, 10, 10, 10]; msg.locateItem.time = -1; });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showTowerLine", data: { lineid: lineid, clear: true } });
    ...
    m.require(["station", "line", "grid", "dcc"], function () {
        var dlls = [];
        dlls.push({ type: 'read', db: 'SGC', sql: (ProjectSGC.Meta.getTypeById(lineid) == "ACLINE" ? "sgc/env:GetAcLineTower" : "sgc/env:GetDcLineTower"), args: [lineid] });
        dlls.push({ type: 'read', db: 'SGAPP', sql: "sgc/icing:GetLineWarn", args: [lineid, st, et] });
        PDP.exec(dlls, function (ret) {
            if (!ret.check("获取线路信息失败", true)) return;
            var towers = ret.value[0];
            var lst = ret.value[1];
            var towerEffect = [];
            Ysh.Array.each(lst, function (row) {
                var stower = Number(row[1]);
                var etower = ((!!row[2]) ? Number(row[2]) : stower);
                if (stower && etower && (!isNaN(stower)) && (!isNaN(etower))) {
                    if (stower == etower)
                        ices.push({ lineID: lineid, startTower: stower - 1, endTower: etower + 1, glow: true });
                    else
                        ices.push({ lineID: lineid, startTower: stower, endTower: etower, glow: true });
                    for (var i = 0; i < towers.length; i++) {
                        var tower = towers[i];
                        var towerno = Number(tower[1]);
                        if ((towerno >= stower) && (towerno <= etower))
                            towerEffect.addSkipSame(tower[0]);
                    }
                }
            });
            var msg = [];//{ type: "lighttower", image: { width: 30, height: 37.5, url: "/textures/coin/Pin.png" }, locateData: [{ lineid: "", towerid: "", data: {} }] };

            for (var i = 0; i < towerEffect.length; i++) {
                msg.push({ lineid: lineid, towerid: towerEffect[i] });
            }
            ...
            ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "ShowGlowForIceLine", selstate: true, data: ices });
            //ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showPartialTowerLine", selstate: true, data: ices });
        });
    });
    return;
...
this.highLightTowers = function (data) {
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "ShowGlowForIceLine", selstate: false });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showPartialTowerLine", selstate: false });
    var m = ProjectSGC.Global.getMainObject("ModelList");
    var ices = [];
    Ysh.Array.each(data, function (row) {
        ...
        if (stower && etower && (!isNaN(stower)) && (!isNaN(etower))) {
            if (stower == etower)
                ices.push({ lineID: lineid, startTower: stower - 1, endTower: etower + 1, glow: false });
            else
                ices.push({ lineID: lineid, startTower: stower, endTower: etower, glow: false });
        }
    });
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "ShowGlowForIceLine", selstate: true, data: ices });
    //ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showPartialTowerLine", selstate: true, data: ices });
...
this.showLineLight = function (id, st, et) {
    ...
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showStationLineByOpacity", selstate: true, data: { lineIDs: [id], stationIDs: ['1'] } }, "icing");
    (ProjectSGC.Global.getMainObject("bottomMenuInst") || {}).noMenu = id;
    ProjectSGC.Map.locate("", id);
    ProjectSGC.Map.hideIcon("icing");
    ProjectSGC.Map.postMessage({ eventType: "menuope", menuname: "showImageIcon", selstate: false, data: { type: "icingtower" } });
    var args = {
    ...
    return;
    PDP.read("SGAPP", "sgc/icing:GetLineTower", [id, st, et], function (data) {
        if (!data.isOK) return;
        ...
        ProjectSGC.Map.postMessage({
            eventType: "menuope", menuname: "showImageIcon", selstate: true, data: { type: "icingtower", paddingtop: -18, images: { width: 30, height: 37.5, imgUrl: "/textures/coin/Pin.png" }, locateData: msg }
        });
...
this.refreshLineData = function (m) {
    ...
    for (var i = 0; i < this.all.length; i++) {
        ...
        var l = m.getLine(id);
        if (!l) continue;
        var state = l[ProjectSGC.LINE.STATE];
        if (state == 1006) {
            this.all.splice(i, 1);
            i--;
            continue;
...
this.refreshLine = function (m) {
    var _this = this;
    PDP.read("SGAPP", "sgc/icing:GetLineStat",[this.st,this.et], function (ret) {
        if (!ret.check("获取覆冰数据", true)) return;
        _this.ds1 = ret.value;
        _this.all = _this.ds1; _this.refreshLineData(m);
    });
    // PDP.load("sgc/icing:GetLineStat",{st:this.st,et:this.et},function(ret){
    //     if (!ret.check("获取覆冰数据", true)) return;
    //     debugger
    //     _this.ds1 = ret.value;
    //     _this.all = _this.ds1; _this.refreshLineData(m);
    // })
    PDP.read("SGAPP", "sgc/icing:GetLineStatEx", [this.st, this.et], function (ret) {
        if (!ret.check("获取覆冰数据", true)) return;
        _this.highLightTowers(ret.value);
    });
...
this.createBar = function () {
    ...
    var vMain = ProjectSGC.Global.getMainObject("vMain");
    if (!vMain) return;
    this.bar = vMain.floatPage("sgc_tq_icing_region", { defaultlevel: this.defaultlevel, defaultcity: this.defaultcity, defaultname: this.defaultname, needrefresh: !(this.initline && this.inittime) ,main:this }, (new Date()).getTime());
}
