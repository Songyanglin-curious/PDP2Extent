...
//根据位置获取element
XmlElement.prototype.getElementByPosition = function (position: number) {
    var element = null;

    // 递归函数用于遍历子元素
    function traverse(node) {
        if (node.type === 'element' && node.startTag?.startPosition <= position && node.endTag?.endPosition >= position) {
            element = node;
        }
